...
<body>
	<div id="container">
		...
		<div class="ct">
			<div class="ct_wrap">
				<div class="top_title">
					<a href="/exhibition/${product.id}" class="btn_back"
					...
					<div class="container_visual" style="width: 414px;">
						<ul class="visual_img">
							<li class="item" style="width: 414px;"><img alt=""
								class="img_thumb" src="/img/${product.fileId}"> <span
								class="img_bg"></span>
								<div class="preview_txt">
									<h2 class="preview_txt_tit">${product.name}</h2>
									<em class="preview_txt_dsc"><fmt:formatNumber value="${prices[0].price}" type="currency" currencySymbol="₩" /> ~ </em><em
						...
						<c:forEach var="price" items="${prices}">
							<div class="qty" data-price="${price.price}">
								...
								<div class="qty_info_icon">
									<strong class="product_amount"> <span> <c:choose>
												<c:when test="${price.priceType eq 1}">
												성인
												</c:when>
												<c:when test="${price.priceType eq 2}">
												유아
												</c:when><c:when test="${price.priceType eq 3}">
												청소년
												</c:when>
											</c:choose>
									...
									</strong> <em class="product_dsc"><fmt:formatNumber value="${price.price}" groupingUsed="true"/>원 (<fmt:formatNumber value="${price.discountRate}" type="percent" groupingUsed="false" /> 할인가)</em>
								</div>
							</div>
						</c:forEach>
								...
								<div class="inline_form">
									...
									<div class="inline_control">
										<input type="text" name="name" id="name" class="text"
											value="${sessionScope.user.username}" maxlength="17">
									</div>
								</div>
								<div class="inline_form">
								...
								<div class="inline_form">
									...
									<div class="inline_control">
										<input type="email" name="email" id="email" class="email"
											value="${sessionScope.user.email}" maxlength="50">
									</div>
								</div>
